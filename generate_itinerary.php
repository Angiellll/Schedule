...
function ensure_array($v){
  ...
  if(is_string($v)){
    $t=json_decode($v,true); if(is_array($t))return $t;
...
function minutes_to_hhmm($m){
  $m=max(0,(int)$m); $h=intdiv($m,60); $mm=$m%60; $h=$h%24; // 簡單防呆
  return sprintf('%02d:%02d',$h,$mm);
}
function time_to_minutes_or_default($t,$def='12:00'){
  $m=hhmm_to_minutes($t); return $m===null?hhmm_to_minutes($def):$m;
}
function earliest_open_minutes_from_string($s){
  if(!$s)return null;
  preg_match_all('/([01]?\d|2[0-3]):([0-5]\d)/',$s,$m);
  if(empty($m[0]))return null;
  $mins=array_map('hhmm_to_minutes',$m[0]);
  $mins=array_values(array_filter($mins,fn($v)=>$v!==null));
  if(empty($mins))return null; sort($mins); return $mins[0];
}
function index_cafes_by_name($cafes){ $idx=[]; foreach($cafes as $c) if(!empty($c['name'])) $idx[$c['name']]=$c; return $idx; }
function enrich_itinerary_with_cafe_fields($it,$idx){
  foreach($it as &$item){
    if(!empty($item['place']) && isset($idx[$item['place']])){
      $c=$idx[$item['place']];
      foreach(['address','mrt','limited_time','socket','pet_friendly','outdoor_seating','minimum_charge'] as $k){
        if(isset($c[$k]) && empty($item[$k])) $item[$k]=$c[$k];
      }
    }
  } unset($item); return $it;
}
function build_tags_from_cafe($c){
  $t=[];
  if(isset($c['limited_time']))$t[]=$c['limited_time']==='0'?'不限時':'限時';
  if(isset($c['socket']))$t[]=$c['socket']==='1'?'有插座':'無插座';
  if(isset($c['pet_friendly']))$t[]=$c['pet_friendly']==='1'?'寵物友善':'非寵物友善';
  if(isset($c['outdoor_seating']))$t[]=$c['outdoor_seating']==='1'?'戶外座位':'無戶外座位';
  if(isset($c['minimum_charge']))$t[]=$c['minimum_charge']==='0'?'無低消':'有低消';
  return $t;
}
function to_candidates($cafes,$limit=5){
  $out=[]; $n=0;
  foreach($cafes as $c){
    $out[]=['name'=>$c['name']??'','address'=>$c['address']??null,'mrt'=>$c['mrt']??null,'tags'=>build_tags_from_cafe($c)];
    if(++$n>=max(3,min(5,$limit))) break;
  }
  return $out;
}
function filter_cafes_open_by_start_strict($cafes,$startHHmm){
  $startMin=hhmm_to_minutes($startHHmm); if($startMin===null)return $cafes;
  $out=[];
  foreach($cafes as $c){
    $e=earliest_open_minutes_from_string($c['open_time']??($c['Open_time']??null));
    if($e===null || $e <= $startMin) $out[]=$c;
  }
  return $out;
}
...
硬性規則：
0) 第一個項目的 time **必須等於 {$startTime}**。若第一個是 cafe，請從「第一時段可用」清單選擇；若清單為空或不用咖啡廳，請改安排具名景點（例：某公園/市場/書店/展館）。
...
/** 1) 把「整天最早的那一筆」時間固定為 $startTime（9:00 / 10:00 / 13:00） */
function force_first_item_time(&$plan, $startTime){
  $it = is_array($plan['itinerary'] ?? null) ? $plan['itinerary'] : [];
  if (empty($it)) return;
  $firstIdx=null; $firstT=PHP_INT_MAX;
  foreach($it as $i=>$item){
    $t=time_to_minutes_or_default($item['time']??'23:59');
    if($t<$firstT){ $firstT=$t; $firstIdx=$i; }
  }
  if($firstIdx!==null) $it[$firstIdx]['time']=$startTime;
  // 重新依時間排序（避免第一筆不在最上面）
...
/**
 * 2) 依 must_include 覆寫「咖啡廳」欄位，但保留 GPT 給的時間：
 *    - 先把既有「咖啡廳項目」逐一用你勾的店名覆寫（數量以兩邊最小值為準）
 *    - 還有剩的店名：把後續的非咖啡項目改成咖啡廳（仍保留該項目的時間）
 *    - 真的不夠放才在結尾追加（以最後一筆時間 + 90 分鐘疊加）
 */
function apply_must_include_preserving_times(&$plan, $must_include, $cafes_all){
  if (empty($must_include)) return;

  $idx = index_cafes_by_name($cafes_all);
  $must = array_values(array_filter($must_include, fn($n)=>isset($idx[$n])));
  if (empty($must)) return;

  $it = is_array($plan['itinerary'] ?? null) ? $plan['itinerary'] : [];
  if (empty($it)) return;

  // 1) 找出所有咖啡廳項目 index（依時間排序）
  usort($it, fn($a,$b)=>time_to_minutes_or_default($a['time']??'12:00') <=> time_to_minutes_or_default($b['time']??'12:00'));
  $cafeIdx=[]; $nonCafeIdx=[];
  foreach ($it as $i=>$x){
    if (strtolower($x['category']??'')==='cafe') $cafeIdx[]=$i; else $nonCafeIdx[]=$i;
  }

  $mi = 0;

  // 2) 用必選覆寫現有咖啡廳項目的「place」
  foreach ($cafeIdx as $i){
    if ($mi >= count($must)) break;
    $name = $must[$mi++];
    $it[$i]['place'] = $name;
    $it[$i]['category'] = 'cafe';
    $t = $it[$i]['time'] ?? '12:00';
    $it[$i]['period'] = (hhmm_to_minutes($t) < hhmm_to_minutes('12:00')) ? 'morning' : ((hhmm_to_minutes($t) < hhmm_to_minutes('18:00')) ? 'afternoon' : 'evening');
    if (empty($it[$i]['activity'])) $it[$i]['activity'] = ($it[$i]['period']==='morning')?'晨間咖啡':'下午咖啡與甜點';
    if (empty($it[$i]['desc']))     $it[$i]['desc']     = '（必選）';
  }

  // 3) 仍有剩的必選 → 把「非咖啡項目」（從第二筆之後優先）改成咖啡廳，但保留時間
  if ($mi < count($must)){
    foreach ($nonCafeIdx as $i){
      // 不動第一筆（多半是起始的固定時間行程，避免破壞你的開場）
      if ($i === 0) continue;
      if ($mi >= count($must)) break;
      $name = $must[$mi++];
      $t = $it[$i]['time'] ?? '12:00';
      ...
      $it[$i]['category'] = 'cafe';
      $it[$i]['period'] = (hhmm_to_minutes($t) < hhmm_to_minutes('12:00')) ? 'morning' : ((hhmm_to_minutes($t) < hhmm_to_minutes('18:00')) ? 'afternoon' : 'evening');
      $it[$i]['activity'] = ($it[$i]['period']==='morning')?'晨間咖啡':'下午咖啡與甜點';
      $it[$i]['desc'] = '（必選）';
    }
  }

  // 4) 還有剩 → 直接追加（以最後一筆 +90 分鐘）
  if ($mi < count($must)){
    $lastMin = 0;
    foreach ($it as $x){ $lastMin = max($lastMin, time_to_minutes_or_default($x['time']??'12:00')); }
    while ($mi < count($must)){
      $lastMin += 90;
      $t = minutes_to_hhmm($lastMin);
      $period = (hhmm_to_minutes($t) < hhmm_to_minutes('12:00')) ? 'morning' : ((hhmm_to_minutes($t) < hhmm_to_minutes('18:00')) ? 'afternoon' : 'evening');
      $name = $must[$mi++];
      $it[] = [
        'time'=>$t,'place'=>$name,'activity'=>($period==='morning')?'晨間咖啡':'傍晚咖啡',
        'transport'=>'步行','period'=>$period,'category'=>'cafe','desc'=>'（必選）'
      ];
    }
  }
...
// 先固定第一筆時間，再覆寫咖啡廳（保留 GPT 的時間分配）
force_first_item_time($llm_json, $startTime);
apply_must_include_preserving_times($llm_json, $must_include, $cafes);
