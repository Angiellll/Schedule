...
function filterCafesByLocation(array $data, string $city, string $district, string $road, array $preferences): array {
    return array_values(array_filter($data, function($cafe) use ($city, $district, $road, $preferences) {
        ...
        foreach ($preferences as $pref) {
            switch($pref) {
                case "不限時": if (empty($cafe['limited_time'])) return false; break;
                case "有插座": if (empty($cafe['socket'])) return false; break;
                case "無低消": if (empty($cafe['minimum_charge'])) return false; break;
                case "寵物友善": if (empty($cafe['pet_friendly'])) return false; break;
                case "戶外座位": if (empty($cafe['outdoor_seating'])) return false; break;
            }
        }
...
try {
    $cafes = loadCafes();
    $results = filterCafesByLocation($cafes, $city, $district, $road_input, $preferences);
